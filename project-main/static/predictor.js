...
function initPredictorForm() {
  ...
  if (form) {
    form.addEventListener('submit', function(e) {
      ...
      // Debug log
      console.log('Form submission - Collected metrics:', metrics);
      ...
      console.log('After parameter name fixing:', metrics);
      ...
      console.log('Sending prediction request with data:', JSON.stringify(metrics));
    ...
    csvUploadBtn.addEventListener('click', function() {
      ...
      input.onchange = function(e) {
        if (e.target.files && e.target.files[0]) {
          ...
          // Send CSV file to server for batch processing
          fetch('/predict_batch', {
            method: 'POST',
            body: uploadForm
          })
          ...
          .then(data => {
            if (data.predictions && data.predictions.length > 0) {
              // Use the first prediction result to populate the form
              const firstResult = data.predictions[0];
              populateFormWithData(firstResult.input);
              displayFuturisticResult(firstResult);
              hideLoading();
              showNotification(`Processed ${data.count} predictions using AI model. Showing first result.`, 'success');
            } else {
          ...
          .catch(error => {
            ...
            populateWithSampleData();
            hideLoading();
            showNotification('CSV processing failed. Using sample data instead.', 'warning');
          });
        }
      };
...
function displayFuturisticResult(data) {
  const resultContainer = document.getElementById('result-container');
  console.log('Displaying result in container:', resultContainer ? 'found' : 'not found');
  console.log('Result data to display:', JSON.stringify(data));
  ...
  console.log('Preparing display with:', { resultClass, resultText, probability });
  ...
  setTimeout(() => {
    console.log('Animating influence bars');
    document.querySelectorAll('.influence-bar').forEach(bar => {
      const targetWidth = bar.dataset.width;
      console.log(`Setting influence bar width to ${targetWidth}`);
    ...
    if (data.is_ai_prediction) {
      ...
      console.log('AI badge found:', aiBadge ? 'yes' : 'no');
      if (aiBadge) {
...
function getResultSummary(data) {
  if (!data) return 'No prediction data available.';
  ...
  if (data.prediction === 1) {
    ...
    const cleanProbability = Math.round((1 - data.probability) * 100);
    return `The code has a ${cleanProbability}% probability of being clean. Good quality detected.`;
  }
}

// Get appropriate class for probability display
